...
class Controller
{
    ...
    public function run($data = null)
    {
        ...
        elseif($this->url[0] == 'requests') {
            if(isAjax()) {
				if($this->url[1] == 'postmessage' || $this->url[1] == 'f_postmessage' || $this->url[1] == 'post_dispute' || $this->url[1] == 'f_post_dispute' || $this->url[1] == 'admin_post_dispute'){
					foreach($data as $d){
						echo $d;
					}
				}else{
			...
			elseif($this->url[0] == CLIENT_URL) :
				if(isset($data['content_api']))
				{
					$data['content_view'] = $data['content_api'];
				} else {
					$data['url']          = $this->url[0];
					$data['header_view']  = $this->getHeader();
