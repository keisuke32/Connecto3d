			...
			$has = $userModel->hasEmail($input->get('email'));
				if($has === true):
					...
					if ($Update == 1) {
						...
						echo json_encode(array('status'=>'true','msg'=>'Send successfully'));
						die();
					} else {
						echo json_encode(array('status'=>'false','msg'=>'Does not save email'));
						die();
					}
				elseif($has === false):
						echo json_encode(array('status'=>'false','msg'=>'Email not available'));
						die();
                endif;	
		}else{
			echo json_encode(array('status'=>'false','msg'=>'send email address'));
			die();
		}
	} 
