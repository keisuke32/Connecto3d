...
class Admin extends Controller
{
    ...
    public function dashboard() {
		...
		$data['jobs'] = $this->model('Common_model')->get('jobs');
		$data['plans'] = $this->model('Common_model')->get('plans');
		$data['job_payments'] = $this->model('Common_model')->get('job_payments');
		...
		if(isset($_POST['edit_details'])){
		 if ($input->exists()) {
			...
			if (!$validation->fails()) {
				...
				$update = $themeModel->update($input->get('title'), $input->get('sub_title'), $input->get('project_search'), $input->get('freelancer_search'), $input->get('id'));
		...
		if(isset($_POST['post_category'])){
		 if ($input->exists()) {
			...
			if (!$validation->fails()) {
				...
				$insert = $categoryModel->add($input->get('name'));
				...
				if(isset($_POST['edit_category'])){
				 if ($input->exists()) {
					...
					if (!$validation->fails()) {
						...
						$update = $categoryModel->update($input->get('name'), $input->get('id'));
	...
	public function plans()
    {
        ...
        return ['content' => $this->view->render($data, 'admin/plans')];
    }
    ...
    public function edit_plan(){
		...
		$data['plans'] = $this->model('Common_model')->get('plans',array('id'=>$_GET['id']));
        return ['content' => $this->view->render($data, 'admin/edit_plan')];
    ...
    public function add_plan(){
        ...
        return ['content' => $this->view->render($data, 'admin/add_plan')];
    ...
    public function update_plan(){
		...
		$id = $_POST['id'];
		$upd['name'] = $_POST['name'];
		$upd['price'] = $_POST['price'];
		$upd['description'] = $_POST['description'];
		$upd['status'] = $_POST['status'];

		$upd['token'] = $_POST['token'];
		$upd['per_token_amount'] = $_POST['per_token_amount'];
		$upd['month'] = $_POST['month'];
		$upd['updated_at'] = date("Y-m-d H:i:s");

		$update = $this->model('Common_model')->update('plans',$upd,array('id'=>$id));
    ...
    public function insert_plan(){
		...
		$insert = $this->model('Common_model')->add('plans',$upd);
    ...
    public function jobs()
    {
		...
		$data['jobs'] = $this->model('Common_model')->get('jobs',["ORDER" => array('updated_at'=>'DESC')]);
		$data['users'] = $this->model('Common_model')->get('user');
		$data['services'] = $this->model('Common_model')->get('services');


        return ['content' => $this->view->render($data, 'admin/jobs')];
    }
    ...
    public function auth_settings()
    {
        ...
        return ['content' => $this->view->render($data, 'admin/admin_users')];
    }
    ...
    public function chat()
    {
		...
		$data['chats'] = $this->model('Common_model')->get('chat_on');

		foreach ($data['chats'] as $key => $ChatVAL) {

			if (0!=$ChatVAL['job_id']) {
				 $job_chat[] = $ChatVAL['job_id'];
			}else{
				$direct_chat[] = $ChatVAL['user_id'];
			}

			
		}

		$data['job_chat'] = array_unique($job_chat);
		$data['direct_chat'] = array_unique($direct_chat);
        ...
        return ['content' => $this->view->render($data, 'admin/chat')];
    }
    ...
    public function chat_on(){
    	...
    	$data['jobs'] = $this->model('Common_model')->get('jobs',['id'=>$job_id]);
    	$data['users'] = $this->model('Common_model')->get('user',['id'=>$client_id]);



    	 return ['content' => $this->view->render($data, 'admin/chat_on')];
    ...
    public function ajax_get_chat(){
    	...
    	if ($job_id=="") {
    		$clause['user_id'] = $client_id;
    		$clause['job_id'] = 0;
    		$chat = $this->model('Common_model')->get('chat_on',$clause);
    	}else{
    ...
    public function ajax_set_chat()
    {
        if (isset($_POST['client_id'])) {
            ...
            $clause['user_id'] = $_POST['client_id'];
            $clause['job_id'] = $_POST['job_id'];
            $clause['message'] = $_POST['message'];
            $clause['created_at'] = date('Y-m-d H:i:s');
            $chat = $this->model('Common_model')->add('chat_on', $clause);
            ...
            if (isset($job[0])) {
                $job = $job[0];
                $title = 'Chat For Job # : ' . $job['job_id'];

                $clients = $this->model('Common_model')->get('user', [
                    'id' =>  $job['user_id']
                ]);
                $client = $clients[0];
                $res = sendCloudMessaseToAndroid($client['firebase_token'], $title, $body, [
                    'job_id' => $job['id'],
                    'job_code' => $job['job_id'],
                    'type' => 'message'
                ]);
    ...
    public function services()
    {
		...
		$data['freelancers'] = $this->model('Common_model')->get('user',$clause);

		$data['services'] = $this->model('Common_model')->get('services');
		$data['service_material'] = $this->model('Common_model')->get('service_material');
		$data['service_type'] = $this->model('Common_model')->get('service_type');
		$data['service_color'] = $this->model('Common_model')->get('service_color');
		$data['service_fininshings'] = $this->model('Common_model')->get('service_fininshings');
		$data['service_infil'] = $this->model('Common_model')->get('service_infil');
		$data['service_layer_height'] = $this->model('Common_model')->get('service_layer_height');
		$data['service_postage'] = $this->model('Common_model')->get('service_postage');

        return ['content' => $this->view->render($data, 'admin/services')];
    }
    ...
    public function subscription()
    {
		...
		$data['subscription'] = $this->model('Common_model')->get('subscription',$clause);

        return ['content' => $this->view->render($data, 'admin/subscription')];
    }
    ...
    public function add_service(){
        ...
        return ['content' => $this->view->render($data, 'admin/add_service')];
    ...
    public function add_tokens(){
		...
		$data['subscription'] = $this->model('Common_model')->get('subscription');

		$clause["ORDER"] = array('id'=>'DESC');
		$clause["user_type"] = 2;
		$data['client'] = $this->model('Common_model')->get('user',$clause);

        return ['content' => $this->view->render($data, 'admin/add_tokens')];
    ...
    public function add_subs(){
        ...
        return ['content' => $this->view->render($data, 'admin/add_subs')];
    ...
    public function insert_service(){
		...
		$clause['freelancer'] = $_POST['freelancer'];
		$check = $this->model('Common_model')->get('services',$clause);

		if (count($check)>0) {
			$_SESSION['message'][] = ['error', "Duplicate Freelancer"];	
		...
		$upd['name'] = $_POST['name'];
		$upd['freelancer'] = $_POST['freelancer'];
		$upd['description'] = $_POST['description'];

		if (isset($_POST['material'])) {
			$upd['material'] = json_encode($_POST['material']);
		}

		if (isset($_POST['type'])) {
			$upd['type'] = json_encode($_POST['type']);
		}

		if (isset($_POST['layer_height'])) {
			$upd['layer_height'] = json_encode($_POST['layer_height']);
		}

		if (isset($_POST['infil'])) {
			$upd['infil'] = json_encode($_POST['infil']);
		}

		if (isset($_POST['fininshings'])) {
			$upd['fininshings'] = json_encode($_POST['fininshings']);
		}

		if (isset($_POST['color'])) {
			$upd['color'] = json_encode($_POST['color']);
		}

		if (isset($_POST['postage'])) {
			$upd['postage'] = json_encode($_POST['postage']);
		}
		...
		$insert = $this->model('Common_model')->add('services',$upd);
    ...
    public function process_store_tokens(){


    	$subscription_id = $_POST['subscription'];

    	$quantity = $_POST['quantity'];
    	$expiry = date($_POST['expiry']." H:i:s");
    	...
    	for ($i=0; $i <$quantity ; $i++) { 
			...
			$tkn['user_id'] = $subscriptionDATA[0]['user_id'];
			$tkn['subscription_id'] = $subscription_id;
			$tkn['payment_id'] = 0;
    ...
    public function process_add_sub(){
		...
		$insert = $this->model('Common_model')->add($table,$upd);
    ...
    public function edit_service(){
		...
		$data['services'] = $this->model('Common_model')->get('services',array('id'=>$_GET['id']));
        return ['content' => $this->view->render($data, 'admin/edit_service')];

    }

    public function edit_service_sub(){
		...
		$data['sub_services'] = $this->model('Common_model')->get($_GET['type'],array('id'=>$_GET['id']));
		$data['sub_service_name'] = $_GET['type'];
        return ['content' => $this->view->render($data, 'admin/edit_service_sub')];
    ...
    public function update_service(){
		...
		$update = $this->model('Common_model')->update('services',$upd,array('id'=>$id));
    ...
    public function update_service_sub(){
		...
		$update = $this->model('Common_model')->update($_POST['table'],$upd,array('id'=>$id));
    ...
    public function freelancers()
    {
        ...
        return ['content' => $this->view->render($data, 'admin/freelancers')];
    }
    ...
    public function clients()
    {
		...
		$clients = $this->model('Common_model')->get('user',$clause);
		$data['plans'] = $this->model('Common_model')->get('plans');

		$rebuild = array();

		foreach ($clients as $key => $clientsVAL) {
			$rebuild[$key] = $clientsVAL;
			$rebuild[$key]['plan_data'] = $this->model('Common_model')->get_plan_data($clientsVAL['id']);
		}

		$data['clients'] = $rebuild;

        return ['content' => $this->view->render($data, 'admin/clients')];
    }
   ...
   // view_job
    ...
    public function view_job()
    {
		...
		$data['job'] = $this->model('Common_model')->get('jobs',array('id'=>$_GET['id']));


		$data['payments'] = $this->model('Common_model')->get('job_payments',array('job_id'=>$_GET['id']));
		...
		$data['job_action'] = $this->model('Common_model')->get('job_actions',array('job_id'=>$_GET['id']));

		

        return ['content' => $this->view->render($data, 'admin/view_job')];
    }
    ...
    public function nda()
    {
        ...
        return ['content' => $this->view->render($data, 'admin/nda')];
    }
    ...
    public function view_subscription(){
		...
		$data['users'] = $this->model('Common_model')->get('user');
		$data['plans'] = $this->model('Common_model')->get('plans');
		$data['subscription'] = $this->model('Common_model')->get('subscription',array('id'=>$_GET['id']));
        return ['content' => $this->view->render($data, 'admin/view_subscription')];
    ...
    public function job_payments(){
		...
		$clause["ORDER"] = array('id'=>'DESC');
		$data['payments'] = $this->model('Common_model')->get('job_payments',$clause);

		$data['token'] = $this->model('Common_model')->get('subscription_tokens');


		$data['users'] = $this->model('Common_model')->get('user');
		$data['jobs'] = $this->model('Common_model')->get('jobs');
        return ['content' => $this->view->render($data, 'admin/job_payments')];
    ...
    function remove_plan(){
    	...
    	$subscription_check = $this->model('Common_model')->get('subscription',array('plan_id'=>$_GET['id']));

    	if (count($subscription_check)>0) {
    		$_SESSION['message'][] = ['error', "Plan used in subscription !"];
    		redirect('admin/plans');
    	}


    	$delete = $this->model('Common_model')->delete('plans',array('id'=>$_GET['id']));
    ...
    function remove_chat(){
    	...
    	if ($job_id=="") {
    		$delete = $this->model('Common_model')->delete('chat_on',array('user_id'=>$client_id));
    	}else{
    		$delete = $this->model('Common_model')->delete('chat_on',array('job_id'=>$job_id));
    	}
    ...
    function remove_job(){
    	...
    	$job_payments = $this->model('Common_model')->get('job_payments',array('job_id'=>$_GET['id']));

    	if (count($job_payments)>0) {
    		$_SESSION['message'][] = ['error', "Job used in job payments !"];
    		redirect('admin/jobs');
    	}


    	$delete = $this->model('Common_model')->delete('jobs',array('id'=>$_GET['id']));
    ...
    function remove_job_payment(){
		...
		if (!isset($_GET['id'])) {
    		$_SESSION['message'][] = ['error', "Invalid Request !"];
    		redirect('admin/job_payments');
    	}

    	$delete = $this->model('Common_model')->delete('job_payments',array('id'=>$_GET['id']));
    	redirect('admin/job_payments');
    ...
    function remove_subscription(){
    	...
    	$delete = $this->model('Common_model')->delete('subscription',array('id'=>$_GET['id']));
    	redirect('admin/subscription');

    }


    function view_job_payment(){
		...
		$data['users'] = $this->model('Common_model')->get('user');
		$data['jobs'] = $this->model('Common_model')->get('jobs');
        return ['content' => $this->view->render($data, 'admin/view_job_payment')];
    ...
    public function token()
    {
		...
		$data['token'] = $this->model('Common_model')->get('subscription_tokens',$clause);
		$data['users'] = $this->model('Common_model')->get('user',array('user_type'=>2));
		$data['payment'] = $this->model('Common_model')->get('job_payments');
		$data['subscription'] = $this->model('Common_model')->get('subscription');

        return ['content' => $this->view->render($data, 'admin/token')];
    }
    ...
    function delete_service(){
    	...
    	$subscription_check = $this->model('Common_model')->get('jobs',array('service_id'=>$_GET['id']));

    	if (count($subscription_check)>0) {
    		$_SESSION['message'][] = ['error', "Service used in Jobs !"];
    		redirect('admin/services');
    	}


    	$delete = $this->model('Common_model')->delete('services',array('id'=>$_GET['id']));
    ...
    public function delete_service_sub(){
    	...
    	$delete = $this->model('Common_model')->delete($table,array('id'=>$id));
    ...
    public function process_update_auth(){
		...
		$upd['password'] = password_hash($_POST['password'], PASSWORD_DEFAULT);


		$update = $this->model('Common_model')->update('admin',$upd,array('id'=>$id));
    ...
    public function process_delete_token(){

    	$token_json = $_POST['token_json'];


    	try {

    		$jsOBJ = json_decode($token_json);
		}
		...
		foreach ($jsOBJ as $jsOBJVAL) {
		$delete = $this->model('Common_model')->delete('subscription_tokens',array('id'=>$jsOBJVAL));
		}
