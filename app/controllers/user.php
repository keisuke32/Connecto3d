...
class User extends Controller
{
	...
	function update_client_api()
	{
        ...
        $shippingAddress = $input->get('shipping_address');
        ...
        $path = 'uploads/admin/users/';
        if ($tmpFilePath != "") {
            $time = time();
            $newFilePath = $path . $time . $_FILES['avatar']['name'];
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $avatar = $time . $_FILES['avatar']['name'];
            }
        }

		$update = $userModel->updateClient($name, $slug, $title, $email, $country, $website, $userId,encrypt_text($shippingAddress), $avatar);
		...
		if (isset($user[0])) {
		    $itemUser = $user[0];
		    $itemUser['shipping_address'] = decrypt_text($user[0]['shipping_address']);
			echo json_encode(array('status'=>'true','msg'=>'successfully', 'data' => $itemUser));
			die();
		}else{
