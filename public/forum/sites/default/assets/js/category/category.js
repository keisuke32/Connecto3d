...
jQuery('document').ready(function ($) {
    ...
    multiselect = {
        deselect: function () {
            $('#codo_topics_list .codo_posts_select_post').prop('checked', false);
            $('#codo_topics_multiselect').hide();
            CODOF.topic_creator.selected_topics = [];
        },
        show_delete_modal: function () {

            $('#codo_multiselect_delete').modal();

            var links = $('#codo_topics_list .codo_posts_select_post:checked').parents('article').find('.codo_topics_topic_title');
        ...
        delete_topics: function () {
            ...
            jQuery.post(codo_defs.url + 'Ajax/topic/deleteAll', {
                token: codo_defs.token,
                isSpam: 'no',
                tids: CODOF.topic_creator.selected_topics
            }, function (resp) {
    ...
    $('#codo_topics_list').on('change', ".codo_posts_select_post", function () {

        var tid = this.id.replace('codo_posts_select_', '');

        if (this.checked) {

            CODOF.topic_creator.selected_topics.push(tid);
        } else {

            var index = CODOF.topic_creator.selected_topics.indexOf(tid);
            if (index > -1) {

                CODOF.topic_creator.selected_topics.splice(index, 1);
            }
        }

        var len = CODOF.topic_creator.selected_topics.length;
    ...
    (CODOF.applySideBarPosition = function () {
        if (CODOF.cache.sideBarMenu.el.length === 0) return ;
