...
class user {
    ...
    public function login() {
        ...
        if($loginBy === "EMAIL") {
            $this->smarty->assign("name_placeholder", _t("email"));
        } else if ($loginBy === "USERNAME") {
            $this->smarty->assign("name_placeholder", _t("username"));
        } else {
            $this->smarty->assign("name_placeholder", _t("username or email"));
        }
    ...
    public function edit_profile($id_being_edited) {
        ...
        foreach ($fields as $field) {
            ...
            if (in_array($field['id'], $fids)) {
                ...
                $value = \CODOF\Filter::msg_safe($_POST['input_' . $field['id']]);
                if ($count > 0) {
    ...
    public function register($do) {
        ...
        if (\CODOF\Util::is_set($_REQUEST, $set_fields) && !\CODOF\Util::is_empty($_REQUEST, $req_fields) && $do) {
            ...
            $register->username = strip_tags(str_replace('"', '&quot;', $_REQUEST['username']));
            $register->name = null; //$_REQUEST['name'];
            $register->password = $_REQUEST["password"];
            $register->mail = strip_tags(str_replace('"', '&quot;', $_REQUEST['mail']));
