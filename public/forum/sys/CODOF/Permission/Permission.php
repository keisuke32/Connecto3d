...
class Permission {
    ...
    /**
     * Updates granted state for permission of a particular role
     * @param $roleId
     * @param $permission
     * @param $granted
     */
    public function updateGeneralPermission($roleId, $permission, $granted) {
    ...
    /**
     * Checks if role has permission. Only applicable to general type permissions since they are not overriden by
     * topics or post based permissions.
     * @param $roleId
     * @param $permission
     * @return int
     */
    public function hasGeneralPermission($roleId, $permission) {

        $permission = \DB::table(PREFIX . 'codo_permissions')
            ->where('rid', '=', $roleId)
            ->where('permission', '=', $permission)
            ->first();
