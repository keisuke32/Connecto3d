...
class Subscriber
{
    ...
    public function registerCoreTypes()
    {
        ...
        $this->addType('new_topic', array(new Subscriber, 'ofCategory'));
        $this->addType('new_reply', array(new Subscriber, 'ofTopic'));
        $this->addType('move_topic', array(new Subscriber, 'ofTopic'));
        $this->addType('vote_up', array(new Subscriber, 'ofReputation'));
        $this->addType('vote_down', array(new Subscriber, 'ofReputation'));
    }
    ...
    /**
     * Get user id of the post which was been voted up/down
     * @param array $args
     * @return array
     */
    public function ofReputation($args)
    {
        $data = $args[0];
        $offset = $args[1];
        // only return array first time when offset is 0. if this is not done, this can go into infinite loop
        // TODO: Is this roundabout way really required?
        if ($offset == 0)
            return [0 => $data->puid];
        else return [];
    }
     ...
     * @param int $tid
     */
    public function toTopic(int $cid, int $tid, $type = 3)
    {
     ...
     * @param int $cid
     */
    public function toCategory(int $cid, $type = 3)
    {
        $cid = (int)$cid;
     ...
     * @return int
     */
    public function levelForTopic(int $tid)
    {
        $tid = (int)$tid;
        $result = \DB::table(PREFIX . 'codo_notify_subscribers')
            ->select('type')
