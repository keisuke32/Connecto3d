...
class Forum
{
     ...
     * @return array
     */
    function generateSecondLevelDeepNesting($cats)
    {
        $nestedCats = [];
        $firstLevel = 0;
        $secondLevel = 0;
        foreach ($cats as $cat) {
            if ($cat->cat_pid == 0) {
                $firstLevel = $cat->cat_id;
                $nestedCats[$cat->cat_id] = $cat;
                $nestedCats[$cat->cat_id]->children = [];
            } else {
                if ($cat->cat_pid == $firstLevel) {
                    $secondLevel = $cat->cat_id;
                    $nestedCats[$firstLevel]->children[$cat->cat_id] = $cat;
                    $nestedCats[$firstLevel]->children[$cat->cat_id]->children = [];
                }
                else {
                    $nestedCats[$firstLevel]->children[$secondLevel]->children[$cat->cat_id] = $cat;
                }
            }
        }
        return $nestedCats;
     ...
     * @return array
     */
    function generateThirdLevelDeepNesting($cats)
    {
        $nestedCats = [];
        $firstLevel = 0;
        $secondLevel = 0;
        ...
        foreach ($cats as $cat) {
            if ($cat->cat_pid == 0) {
                ...
                else if ($cat->cat_pid == $secondLevel) {
                    $thirdLevel = $cat->cat_id;
                    $nestedCats[$firstLevel]->children[$secondLevel]->children[$cat->cat_id] = $cat;
                    $nestedCats[$firstLevel]->children[$secondLevel]->children[$cat->cat_id]->children = [];
                } else {
                    $nestedCats[$firstLevel]->children[$secondLevel]->children[$thirdLevel]->children[$cat->cat_id] = $cat;
                }
            }
        }
        return $nestedCats;
