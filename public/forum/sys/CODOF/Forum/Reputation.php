...
class Reputation
{
    ...
    public function up($topic_id, $post_id)
    {
        ...
        if ($this->can_up($pid, $puid)) {
            ...
            $topicData = array(
                ...
                "pid" => $pid,
                "puid" => $puid,
                "topicCreatorUid" => $info['tuid'],
                "notifyFrom" => \CODOF\User\CurrentUser\CurrentUser::id(),
                "notification" => "%actor% voted up your post",
                "bindings" => array()
            );

            $notifier = new \CODOF\Forum\Notification\Notifier();
            $notifier->queueNotify('vote_up', $topicData, $tid);
    ...
    public function down($topic_id, $post_id)
    {
        ...
        if ($this->can_down($pid, $puid)) {
            ...
            if (!$has_rep) {
                // because if it has rep, it will cancel out anyway
                \DB::table(PREFIX . 'codo_reputation')
                    ->insert(array(
                        'from_id' => $user->id,
                        'to_id' => $puid,
            ...
            $topicData = array(
                ...
                "topicCreatorUid" => $info['tuid'],
                "notifyFrom" => \CODOF\User\CurrentUser\CurrentUser::id(),
                "notification" => "%actor% voted down your post",
                "bindings" => array()
            );

            $notifier = new \CODOF\Forum\Notification\Notifier();
            $notifier->queueNotify('vote_down', $topicData, $tid);
    ...
    public function can_down($pid, $to_id)
    {
        ...
        return $this->rule1 && $this->rule2 && $this->rule3 && $this->rule4 && $this->rule5 && $user->can('rep down');
    }
