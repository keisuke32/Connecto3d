...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'reorder' && CODOF\Access\CSRF::valid($_POST['CSRF_token'])) {
        ...
        if (isset($_POST['mode']) && $_POST['mode'] == 'edit' && CODOF\Access\CSRF::valid($_POST['CSRF_token'])) {
            ...
            $arr[":cat_img"] = uploadFile($_GET['cat_id']);
            $arr[':default_subscription_type'] = $_POST['default_subscription_type'] == 'yes' ?
                \CODOF\Forum\Notification\Subscriber::$NOTIFIED : \CODOF\Forum\Notification\Subscriber::$DEFAULT;

            $stmt = $db->prepare($query . $imgql . $cond);
            $stmt->execute($arr);
            $smarty->assign('msg', 'Category updated successfully!');
        }
...
if (isset($_POST['mode'])) {
    ...
    if ($_POST['mode'] == 'new' && CODOF\Access\CSRF::valid($_POST['CSRF_token'])) {
        ...
        $qry = 'INSERT INTO ' . PREFIX . 'codo_categories'
            . '(cat_pid,cat_name,cat_alias,cat_description,cat_img,no_topics,no_posts,cat_order, is_label, default_subscription_type)'
            . 'VALUES(:cat_pid,:cat_name,:cat_alias,:cat_description,:cat_img,:no_topics,:no_posts,:cat_order, :is_label, :default_subscription_type)';
        $stmt = $db->prepare($qry);

        $arr[":cat_pid"] = 0;
        $arr[":cat_name"] = $_POST['cat_name'];
        $arr[":cat_alias"] = CODOF\Filter::URL_safe($_POST['cat_name']); //
        $arr[":cat_img"] = uploadFile(null); //
        $arr[":cat_description"] = $_POST['cat_description'];
        $arr[":no_topics"] = 0;
        $arr[":no_posts"] = 0;
        $arr[":cat_order"] = 0;
...
{
    if (!isset($_FILES['cat_img']) || !isset($_FILES['cat_img']['name']) || $_FILES['cat_img']['name'] == "") {
        if ($id != null) {
            $query = "SELECT * FROM " . PREFIX . "codo_categories WHERE cat_id=:cat_id";
            $db = \DB::getPDO();
            $stmt = $db->prepare($query);
    ...
    if (\CODOF\File\Upload::image($file)) {
