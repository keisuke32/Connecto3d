...
//safe
    ...
    Request::get('Ajax/subscribe/:cid/:level', function (int $cid, int $level) {
        $subscribe = new CODOF\Forum\Notification\Subscriber();
        $subscribe->toCategory($cid, $level);
    });

    Request::get('Ajax/subscribe/:cid/:tid/:level', function (int $cid, int $tid, int $level) {
        $subscribe = new CODOF\Forum\Notification\Subscriber();
