...
<html>
	<head>
		...
		<?php foreach(['base', 'bootstrap.min', 'font-awesome.min', 'feathericon.min', 'login', 'style-old'] as $css): ?>
		...
		<?php if($data['url'] == 'login'): ?>
			...
			<?php foreach(['bootstrap.min', 'wave'] as $js): ?>
		...
		<?php else: ?>
			...
			<?php foreach(['bootstrap.min', 'app.min'] as $js): ?>
				<script type="text/javascript" src="<?=$this->siteUrl()?>/<?=$this->themePath()?>/<?=$this->theme()?>/assets/js/<?=$js?>.js"></script>
			<?php endforeach ?>
